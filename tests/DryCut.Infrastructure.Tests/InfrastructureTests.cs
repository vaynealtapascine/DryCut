...
namespace DryCut.Infrastructure.Tests;
...
public sealed class InfrastructureTests
{
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), "DryCut-tests", Guid.NewGuid().ToString("N"));
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "drycut-tests", Guid.NewGuid().ToString("N"), "settings.json");
    ...
    {
        var folder = Path.Combine(Path.GetTempPath(), "drycut-tests", Guid.NewGuid().ToString("N"));
    ...
    [Fact]
    public async Task RealModelSmokeSkipsUnlessDrycutTestModelIsSet()
    {
        var model = Environment.GetEnvironmentVariable("DRYCUT_TEST_MODEL");
        if (string.IsNullOrWhiteSpace(model))
        {
            Console.WriteLine("SKIPPED — env var DRYCUT_TEST_MODEL not set; the real-model smoke test did not run.");
            return;
        }
        var imagePath = Path.Combine(Path.GetTempPath(), "drycut-smoke.png");
    ...
    [Fact]
    public async Task StrongModelSmokeSkipsUnlessDrycutTestStrongModelIsSet()
    {
        var model = Environment.GetEnvironmentVariable("DRYCUT_TEST_STRONG_MODEL");
        if (string.IsNullOrWhiteSpace(model))
        {
            Console.WriteLine("SKIPPED — env var DRYCUT_TEST_STRONG_MODEL not set; the strong-model smoke test did not run.");
            return;
        }
        var imagePath = Path.Combine(Path.GetTempPath(), "drycut-strong-smoke.png");
